...
- **Env vars reset to `.env` values on deploy** — `twilio serverless:deploy` overwrites all runtime env vars with the `.env` file values. Keep `.env` up to date with real values (not placeholders). If `.env` has the correct values, no post-deploy fixup is needed. If you use `serverless:env:set` to change a value at runtime, update `.env` too or it will be lost on next deploy.
